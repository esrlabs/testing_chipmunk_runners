...
if ARGV[0].start_with?('PR~')
  ...
  pull_request = client.pull_request("#{REPO_OWNER}/#{REPO_NAME}", pr_number)
...
def process_release_or_pr(branch_or_tag_name, identifier)
  ...
  Dir.mktmpdir do |temp_dir|
    begin
      ...
      system("git clone --depth 1 --branch #{branch_or_tag_name} https://github.com/#{REPO_OWNER}/#{REPO_NAME}.git #{temp_dir}")
